...
//lwip DHCP任务
//设置任务优先级
#define LWIP_DHCP_TASK_PRIO		20
//任务堆栈大小	
#define LWIP_DHCP_STK_SIZE 		300  
...
u8 lwip_comm_mem_malloc(void)
{
	...
	TCPIP_THREAD_Task_Handler=mymalloc(SRAMEX,TCPIP_THREAD_STACKSIZE*4);//给内核任务申请堆栈 
	LWIP_DHCP_TASK_Handler=mymalloc(SRAMEX,LWIP_DHCP_STK_SIZE*4);				 //给dhcp任务堆栈申请内存空间
...
{ 	
	...
	myfree(SRAMEX,TCPIP_THREAD_Task_Handler);
	myfree(SRAMEX,LWIP_DHCP_TASK_Handler);
}
//lwip 默认IP设置
//lwipx:lwip控制结构体指针
