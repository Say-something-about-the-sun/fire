...
//void sqtt_send(void *pvParameters);
//void su_chuli(void *pvParameters);
/////************************************************************************
////** 函数名称: MQTT_Connect								
////** 函数功能: 初始化客户端并登录服务器
...
//    int buflen = sizeof(buf);
//    int len = 0;
//    data.clientID.cstring = CLIENT_ID02;                   //随机
//    data.keepAliveInterval = KEEPLIVE_TIME;         //保持活跃
//    data.username.cstring = USER_NAME02;              //用户名
//    data.password.cstring = PASSWORD02;               //秘钥
//    data.MQTTVersion = MQTT_VERSION;                //3表示3.1版本，4表示3.11版本
//    data.cleansession = 1;
//    //组装消息
//    len = MQTTSerialize_connect((unsigned char *)buf, buflen, &data);
...
//    if(MQTTSubscribe(MQTT_Socket,(char *)TOPIC,QOS0) < 0)
//    {
//         //重连服务器
...
//#if  LWIP_DNS
//    ip_addr_t dns_ip;
//    netconn_gethostbyname(HOST_NAME02, &dns_ip);
//    host_ip = ip_ntoa(&dns_ip);
//    PRINT_DEBUG("host name : %s , host_ip : %s\n",HOST_NAME,host_ip);
//#else
//    host_ip = HOST_NAME02;
...
//    if(MQTTSubscribe(MQTT_Socket,(char *)TOPIC02,QOS0) < 0)
//    {
//         //重连服务器
//         PRINT_DEBUG("客户端订阅消息失败...\n");
//          //关闭链接
...
//            
//            //心跳成功
//            PRINT_DEBUG("01发送保持活性ping作为心跳成功....\n");
//            //表明有数据交换
//            no_mqtt_msg_exchange = 0;
//        } 
//         
...
//////	  recv_data->temperature=1.36;
//////    recv_data->humidity=2.33;
////    //初始化json数据
////    cJSON* cJSON_Data = NULL;
////    cJSON_Data = cJSON_Data_Init();
////	  double a,b;
//////  const  char *a="{\"id\": \"678\",\"version\": \"1.0\",\"params\": {\"Power\":cs01:cs01\":569},\"method\": \"thing.event.property.post\"}";
////MQTT_SEND_START:
////  
////    while(1)
////    {
...
////            //发布消息
////            ret = MQTTMsgPublish(MQTT_Socket,(char*)TOPP,QOS0,(uint8_t*)p);
////					  
////            if(ret >= 0)
////            {
////                //表明有数据交换
...
//            res=cJSON_Update(cJSON_NUM, DEFAULT_PARAMS, &kk);//更新湿度							
...
////							printf("%s\n  p=%d\n",p,sizeof(cJSON_Print(cJSON_Data)));
//							ret = MQTTMsgPublish(MQTT_Socket,(char *)TOPP,QOS0,(uint8_t *)p);//推送消息
//							if(ret >= 0)
//							{
//									//表明有数据交换
//									 no_mqtt_msg_exchange = 0;
...
//void sqtt_heart(void *pvParameters)
//{
// MQTT_SENDddd:
//   while(1)
//	 {
...
////		{
////			vTaskDelay(10);
////      etlhh_input(&lwip_netif);
////		}

////}
//xTaskHandle mqtt_thread_Task_Handler;
//xTaskHandle sqtt_heart_Task_Handler;
//u8 mqtt_thread_init(void)
//{
//	int res;
//	int ree;
//	res=xTaskCreate((TaskFunction_t )mqtt_thread,            //任务函数
//              (const char*    )"mqtt_thread",          //任务名称
//              (uint16_t       )1024,        //任务堆栈大小
//              (void*          )NULL,                  //传递给任务函数的参数
//              (UBaseType_t    )7,       //任务优先级
//              (TaskHandle_t*  )&mqtt_thread_Task_Handler); 
//							
////		ree=xTaskCreate((TaskFunction_t )sqtt_heart,            //任务函数
////              (const char*    )"sqtt_heart",          //任务名称
////              (uint16_t       )500,        //任务堆栈大小
////              (void*          )NULL,                  //传递给任务函数的参数
////              (UBaseType_t    )6,       //任务优先级
////              (TaskHandle_t*  )&sqtt_heart_Task_Handler); 
////  res=sys_thread_new("mqtt_thread", mqtt_thread, NULL, 1024, 6);
////  ree=sys_thread_new("sqtt_send", sqtt_send, NULL, 1024, 4);
////	ree=sys_thread_new("sqtt_heart", sqtt_heart, NULL, 500, 7);
////	ree=sys_thread_new("su_chuli", su_chuli, NULL, 1024, 5);
//	    if(res==pdPASS)							
//       return 0;
//    else		
//	     return 1;
