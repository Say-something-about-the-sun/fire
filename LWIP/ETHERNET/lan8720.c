...
extern xSemaphoreHandle t_xSemaphore;  
...
void ETHERNET_NVICConfiguration(void)
{
	...
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0X05;  //中断寄存器组2最高优先级
...
u8 LAN8720_Get_Speed(void)
{
	...
	u8 speed;
	speed = ((ETH_ReadPHYRegister(0x00,16)&0x06)>>1);
	return speed;
...
u8 ETH_MACDMA_Config(void)
{
	...
	while (ETH_GetSoftwareResetStatus() == SET);//等待软件重启网络完成  该位自动清零
...
//  xSemaphoreTake(t_xSemaphore, portMAX_DELAY );
