...
static u8 CardType=SDIO_STD_CAPACITY_SD_CARD_V1_1;	//SD卡类型，默认为1.x版本
static u32 CSD_Tab[4],CID_Tab[4],RCA=0;					//SD卡的CSD,CID以及相关的发送数据(RCA)数组
static u8 DeviceMode=SD_DMA_MODE;		   				//操作模式,注意,操作模式只能通过SD_SetDeviceMode修改,不能直接修改.这里只是定义一个默认上电的模式(SD_DMA_MODE)
static u8 StopCondition=0; 								//是否发送停止指令标志位,DMA读写时用得到  
volatile SD_Error TransferError=SD_OK;					//数据传输错误标志,DMA读写时使用	    
volatile u8 TransferEnd=0;								//传输结束标志,DMA读写时使用
SD_CardInfo SDCardInfo;									//SD卡信息

//SD_ReadDisk/SD_WriteDisk函数专用buf,用于读写SD卡时的数据缓冲,这个缓冲区必须4字节对齐,在DMA读写时,需要用到这个缓冲区.
